...
git commit -m "Fixes/1678-Issues in admin dashboard

Tasks done
---

- Fixed logout issue from navigation bar
- Fixed continuous rendering of loading spinner while filtering products
"
```
...
git commit -m "Features/1700-Integrate paypal into the system for payment after purchase
...
- Add functionality to navigate to /ticket/download page after ticket purchase
- Add functionality to download the move ticket automatically after visiting the page
"
```
...
- Add functionality to provide custom branding options to the users
- Add functionality to implement the custom branding upon change
...
- Add and design toggle button to toggle between light and dark mode
- Add functionality to toggle between light and dark mode after button click
"
```
...
- Always write the commit in direct manner without any filler words and should write a commit that described the context you changed with your code.
...
- If there is no need for any commit body and only title is capable of explaining the context, you should not use one. This can be true for obvious work done which every developer is capable to understand.

```
// Example which does not need commit body.

git commit -m "Initialize project according to company standard"
```
...
Whenever we complete our task or certain portion of the task, we create a merge request for those changes which we made. So, the merge request should be informative about the overall work done in the project. This will help other team members to get insights into your work and helps reviewer to review the code according to the scope of the work done. Creating an informative merge request also helps to keep the history of the tasks done during the development of the project by certain developer. This will be more effective and easy to follow if there is certain standard while creating the merge request. So, here at Truemark, we do follow standard while creating merge requests and now let's look into the convention of creating a merge request while following our standard.

### Merge Requests Convention

[**Trello/Jira Card Link**](/docs/guidelines/git#trello/jira-card-link)

- Link to the project management tool (Trello/Jira) card.

[**Tasks Completed**](/docs/guidelines/git#tasks-completed)

- List of tasks done for particular feature or bug fix in the project.

[**Remaining Tasks**](/docs/guidelines/git#remaining-tasks)

- List of remaining tasks to be done for feature or bug fix.

[**Steps To Test Features**](/docs/guidelines/git#steps-to-test-features)

- Stepwise walk through to test the certain implemented feature in the project.

[**Screenshots/Video**](/docs/guidelines/git#screenshots/videos)

- Screenshots to show the UI change in the project or video attachment to show the walk through of the feature implemented.

#### Trello/Jira Card Link

We include the project management tool's card link assigned for certain feature or bug fix at first while creating the merge request. Including Trello/Jira card link in the merge request allows team members and project manager to look at the task developer is working on which can make team effective and efficient while assigning task to the certain developer in the coming days.

#### Tasks Completed

This is the second section we should have in our merge requests. This section should include the list of tasks the developer has worked on while integrating certain feature or working on some bug fix in the project.

#### Remaining Tasks

We always can not create a merge request after completing the whole task as we should send our code for review to the senior developers. So, there might be some tasks which contains other sub-tasks that are yet to be completed. Therefore, these remaining sub-tasks are included in the third section while creating the merge request inside of the Remaining Tasks section.

#### Steps To Test Features

This should be included only when working on the frontend part of the application as this section should contain stepwise description to test the certain feature which is implemented. This basically should contain the descriptive feature walk through so it can be easier for the tester to test the feature correctly.

#### Screenshots/Videos

This section is also only included when creating merge request for frontend changes as this should include the UI changes done as screenshots and video attachment can be used to walk through the feature implemented while performing the task.

### Creating A Merge Request

Now, as we have looked at the convention while creating the merge request, lets take a look at creating one of those. There is different convention for creating merge request for frontend and backend task.

#### Merge request for backend

Here is the overview of the Markdown (MD) format we should have for backend merge request.

```md
Trello/Jira Card Link

- Link to the project management tool's card link for certain task.
- Another link for the task's card if task is divided into multiple cards.

Tasks Completed

- Brief about the task 1 done.
- Brief about the task 2 done.
- Brief about the task 3 done.
- and so on...

Remaining Tasks

- Brief about the remaining task 1.
- Brief about the remaining task 2.
- and so on...
```

Lets see an example for the merge request.

```
Trello/Jira Card Link

- https://trello.com/c/DcXinxqG/1385-customer-should-be-able-to-upload-profile-picture

Tasks Completed

- Added functionality and created an API to upload the profile image
- Added functionality and created an API to view the profile image
- Added functionality and created an API to update the profile image
- Added functionality and created an API to remove the profile image
- Added functionality to make the profile image dimension of 125x125 and 250x250.

Remaining Tasks

- Add validation for restricting user to upload image below 5MB.
- Add validation to accept the image in either png or svg format only.
```

#### Merge request for frontend
...
- Step 1 to test feature X
- Step 2 to test feature X
- Step 3 to test feature X
- and so on...

Screenshots/Video

- Description about the UI change 1

  - Screenshot/Video for the UI change

- Description about the UI change 2

  - Screenshot/Video for the UI change 3

- Description about the UI change 4
...
- https://trello.com/c/462gWRAG/1070-updates-in-side-menu-bar-in-ecommerce-app

Tasks Completed

- Added header section in the sidebar which contains logo of the application.
- Changed the home icon according to the design.
- Added left chevron icon in the sidebar.
- Added different links to navigate into different parts of the application.

Remaining Tasks

- Add functionality to close the sidebar when chevron icon is clicked.
- Add sub-menu and implement the design and animation as required while revealing the items in sub-menu.
- Design the links present in the sidebar and add relevant route to them.
- Refactor the code to meet the coding standards followed.

Steps To Test Features

- Go to the dashboard page by visiting /home/dashboard url.
- You will see the right chevron icon in the navigation bar of the application.
- Click on the right chevron icon.
- Sidebar will be revealed.
- You will see the new home icon and various links for the application.

Screenshots/Video

- UI change in the sidebar and home icon
...
There can be different scenarios while creating a merge request for a certain PR. There can be situation when tasks are remained to complete or all tasks have been completed and for each situation, there can be different merge request. Now, lets take a look at the types of merge requests relying upon the task done.

#### Merge Request In Work In Progress (WIP) State

The merge request can contain some tasks which are remained to be done. This state of merge request is also often called as WIP state where certain portion of the tasks are completed but there are still some remaining task to be done. This section adds new section as `Remaining Tasks` in the merge request Markdown(MD) that contains the list of remaining task to be done as will as the title while creating the merge request should start with `WIP` keyword. Overview of the MD format for merge request in WIP state is given below:
...
- List of remaining tasks to be done

Steps To Test Features

- Stepwise walkthrough of the feature implemented (for frontend)

Screenshots/Video

- Description and screenshot/video of the UI changes (for frontend)
```

#### Completed Merge Request

In this type of merge request, all works are completed and the feature or certain functionality is working correctly. We basically omit the `Remaining Task` section in this type of merge request while creating Markdown (MD) for it. It covers all the tasks that are completed and adds steps to test features and screenshots/videos walk through of the feature if the changes are made in the frontend part of the application. Overview of the MD format for completed merge request is given below:
...
1. Review code changes in the merge request so that you can fix issues and typos if you find any
2. Ask your peer and supervisor to review your merge request
...
Before any PR is merged it should also be verified by the Team Lead once to make sure that businees domain and project related standards are maintained properly. Peer and supervisor might not be working in the same project as the reviewee which means they will only review code standards and quality so this step is necessary to avoid other project related nuances and issues that any change could bring in production.
