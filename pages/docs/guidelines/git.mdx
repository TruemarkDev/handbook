...
It should follow the following convention while writing the commit message:
...
Example for the git commit rule:
...
From above commit message, were you able to figure out what is going on there? It says `searching functionality` as commit title, but in which task the developer is working on and in which project? and what are we searching here? persons? birds? or cars? We are in a confusion about what this commit says. Now, let's move on to another section of commit which is describing the tasks done. It says `add input` but what is the input for and what value does it get? Moving on to another task, it says `validate it` but which input is it validating? and what is it validating? So, analyzing the scenario, we can infer that we should always make our commit clear and concise. Many developers who work on legacy project need to have understanding about the codebase when they are working on upgrading, but these kind of commit messages make those developers confused and they cannot infer anything from these. So, for making our project and codebase future proof, we should have our commit messages clean and clear.
