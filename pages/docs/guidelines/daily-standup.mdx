...
There can be scenarios where the task you are doing is blocked due to different reasons. All the reasons that resulted in your task being blocked will go in this section. This is a great way to justify your delay in task if you are questioned about it. Blockers in daily standup messages are the great way to convey message to certain team member about the problem you are facing in your task due to them. There might be different technologies and techniques involved while working on certain project, and different team members might be working on different parts of the application. If your task is being blocked due to some task not being done by other team members, you can post it as a blocker in your daily standup which can help in alerting the team member who is causing the blocker and work the task with highest priority the next time he is working on the project.
...
---
  ...
  - Unclear about the exact functionality about the course filter API which I need to filter courses according to the given user's rating tomorrow.
```
