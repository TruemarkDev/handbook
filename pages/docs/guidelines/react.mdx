...
We manage our codebase in the local environment but we can not always work in a team by working locally. So, we need to push our changes to the cloud, to be more specific - a code hosting service. Therefore, while we check for the standard and source code errors in the local environment, we can also check for the lack of coding standards while the code is hosted in the cloud using review dog.
